...
class NavigationPreferences(context: Context) {
    ...
    var home: Place?
        get() = readPlace(KEY_HOME)
        set(value) = writePlace(KEY_HOME, value)

    var work: Place?
        get() = readPlace(KEY_WORK)
    ...
    fun initializeDemoDefaultsAndRetireLocalTheme() {
        val editor =
            preferences.edit()
                .remove(
        ...
        if (!preferences.contains(KEY_HOME)) {
            editor.putString(
                KEY_HOME,
        ...
        if (!preferences.contains(KEY_WORK)) {
            editor.putString(
                KEY_WORK,
                NavigationJson.placeToJson(
    ...
    fun addRecent(place: Place) {
        ...
        val json = JSONArray()
        updated.forEach { json.put(NavigationJson.placeToJson(it)) }
    ...
    fun saveSafeScreen(screen: NavigationScreen) {
        if (screen in SAFE_STARTUP_SCREENS) {
            preferences.edit()
                .putString(KEY_SAFE_SCREEN, screen.name)
                .apply()
        }
    }
    ...
    fun lastSafeScreen(): NavigationScreen =
        runCatching {
            ...
            .takeIf { it in SAFE_STARTUP_SCREENS }
    ...
    private fun readPlace(key: String): Place? {
        ...
        return runCatching {
            NavigationJson.placeFromJson(JSONObject(stored))
        }.getOrNull()
    }
    ...
    private fun writePlace(
        ...
        preferences.edit().apply {
            if (place == null) {
                ...
                putString(
                    key,
                    NavigationJson.placeToJson(place).toString()
                )
            }
        }.apply()
    }
