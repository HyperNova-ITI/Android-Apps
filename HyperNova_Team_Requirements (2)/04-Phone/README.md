...
```text
Repository: HyperNova_Phone
Package / applicationId: com.hypernova.phone
Namespace: com.hypernova.phone
Public open action: com.hypernova.phone.action.OPEN
...
Own paired-phone connectivity, contact browsing, recent calls, dialing, and call-state UI for the IVI.
...
Launcher card
    ...
    -> com.hypernova.phone/.MainActivity
    -> Full-screen HyperNova Phone
```
...
<activity
    ...
    <intent-filter>
        <action android:name="com.hypernova.phone.action.OPEN" />
...
- Active call.
- Call ended.
- Contacts permission/data unavailable.
...
- Use real Bluetooth and telephony state.
- Plan HFP client behavior for call audio/control.
- Plan PBAP client behavior for contacts and call history.
- Do not copy contacts into logs.
- Keep call state in a repository independent of the Activity.
- Support answer, reject, end call, mute, and audio-route controls only when the platform exposes them.
- Show clear disconnected and unsupported states.
- Do not display a fake recent contact.

---

## 6. Permissions and platform access

- `Bluetooth runtime permissions required by the target API and selected profiles.`
- `Contact access only when a real local/provider source is used.`
- `AAOS Bluetooth HFP/PBAP profile control may require system or privileged integration.`
...
- `ui/ — disconnected, recents, contacts, and call screens.`
- `domain/ — call, contact, connected-device models and use cases.`
- `data/ — Bluetooth profile adapters and contact repository.`
- `service/ — launcher-facing phone status service.`
...
```text
Service class: com.hypernova.phone.service.PhoneStatusService
Bind action: com.hypernova.phone.action.BIND_STATUS
```
...
The Launcher must receive only the minimum glanceable call/contact summary. Full contact browsing remains inside HyperNova Phone.
...
```bash
adb shell pm list packages | grep com.hypernova.phone
```
