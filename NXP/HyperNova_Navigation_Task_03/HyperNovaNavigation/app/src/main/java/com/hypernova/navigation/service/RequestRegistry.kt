...
data class RequestCompletion<C>(
...
class RequestRegistry<R, C>(
    ...
    private val entries =
        linkedMapOf<RequestKey, Entry<R, C>>()
    ...
    fun begin(
        ...
        if (existing != null) {
            if (existing.fingerprint != fingerprint) {
                return RequestRegistration.Conflict
            }

            existing.finalResult?.let {
                return RequestRegistration.Completed(it)
            }

            if (callback !in existing.callbacks) {
    ...
    fun complete(
        ...
        return RequestCompletion(callbacks)
    }
    ...
    private fun removeExpired() {
        ...
        entries.entries.removeAll { (_, entry) ->
            val retainedFrom =
                entry.completedAtMillis
                    ?: entry.createdAtMillis
            now - retainedFrom >= retentionMillis
        }
    }
